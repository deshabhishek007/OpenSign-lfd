...
export default async function GenerateCertificate(docDetails) {
  ...
  // Reserve ~45px at the bottom of every page for the compliance/attribution
  // footer that is drawn just before pdfDoc.save() (see the getPages() loop).
  const minY = startY + 45;
  ...
  // Footer drawn on every page: Indian-law compliance statement + attribution.
  const footerSize = 8;
  const footerColor = rgb(0.35, 0.35, 0.35);
  const footerLine1 =
    'Electronic signatures on this document are legally valid and compliant with the Information Technology Act, 2000 of India.';
  const footerLine2 = 'This Certificate is generated by Legal Data Forensic Sign tool.';
  const drawCentered = (p, txt, y) => {
    const w = timesRomanFont.widthOfTextAtSize(txt, footerSize);
    p.drawText(txt, {
      x: (p.getWidth() - w) / 2,
      y,
      size: footerSize,
  ...
  for (const p of pdfDoc.getPages()) {
    p.drawLine({
      start: { x: 30, y: 44 },
      end: { x: p.getWidth() - 30, y: 44 },
      color: rgb(0.6, 0.6, 0.6),
      thickness: 0.5,
    });
    drawCentered(p, footerLine1, 32);
    drawCentered(p, footerLine2, 21);
  }
